...
[CustomEditor(typeof(HealthDamager))]
public class HealthDamagerEditor : Editor
{
    static BoxBoundsHandle s_BoxBoundsHandle = new();
    ...
    void OnSceneGUI()
    {
        HealthDamager healthDamager = (HealthDamager)target;

        if (!healthDamager.enabled)
            return;

        Matrix4x4 handleMatrix = healthDamager.transform.localToWorldMatrix;
        ...
        handleMatrix.SetRow(2, new Vector4(0f, 0f, 1f, healthDamager.transform.position.z));
        using (new Handles.DrawingScope(handleMatrix))
        {
            s_BoxBoundsHandle.center = healthDamager.offset;
            s_BoxBoundsHandle.size = healthDamager.size;
            ...
            {
                Undo.RecordObject(healthDamager, "Modify Damager");

                healthDamager.size = s_BoxBoundsHandle.size;
                healthDamager.offset = s_BoxBoundsHandle.center;
            }
        }
    }
}
