...
{
    static readonly ContactPoint2D[] s_ContactPointBuffer = new ContactPoint2D[16];
    static readonly Dictionary<Collider2D, Pushable> s_PushableCache = new();
    ...
    protected void CheckGrounded()
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            if (s_ContactPointBuffer[i].normal.y > 0.9f)
            {
                ...
                if (s_PushableCache.TryGetValue(s_ContactPointBuffer[i].collider, out Pushable pushable))
                {
