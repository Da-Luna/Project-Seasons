...
public class VFX
{
    ...
    [System.NonSerialized] public Dictionary<string, VFXInstancePool> vfxOverrideDictionnary;
}
...
public class VFXController : MonoBehaviour
{
    ...
    readonly Dictionary<int, VFX> m_FxPools = new();
    readonly PriorityQueue<VFXInstance> m_RunningFx = new();
    readonly PriorityQueue<PendingVFX> m_PendingFx = new();
    ...
    public void Awake()
    {
        ...
        foreach (var vfx in vfxConfig)
        {
            ...
            vfx.vfxOverrideDictionnary = new Dictionary<string, VFXInstancePool>();
            for (int i = 0; i < vfx.vfxOverride.Length; ++i)
            {
                GameObject obj = new("vfxOverride");
                obj.transform.SetParent(transform);
                vfx.vfxOverrideDictionnary[vfx.vfxOverride[i].prefab.name] = obj.AddComponent<VFXInstancePool>();
                vfx.vfxOverrideDictionnary[vfx.vfxOverride[i].prefab.name].initialPoolCount = 2;
                vfx.vfxOverrideDictionnary[vfx.vfxOverride[i].prefab.name].prefab = vfx.vfxOverride[i].prefab;
            }
    ...
    {
        if (!m_FxPools.TryGetValue(hash, out VFX vfx))
        {
    ...
    void CreateInstance(VFX vfx, Vector4 position, bool flip, Transform parent)
    {
        var instance = vfx.pool.Pop();
