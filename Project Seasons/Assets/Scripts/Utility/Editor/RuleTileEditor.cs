...
namespace UnityEditor
{
	...
	internal class RuleTileEditor : Editor
	{
		...
		public void OnEnable()
		{
			...
			m_ReorderableList = new(Tile.m_TilingRules, typeof(RuleTile.TilingRule), true, true, true, true);
		...
		private void OnDrawElement(Rect rect, int index, bool isactive, bool isfocused)
		{
			...
			Rect matrixRect = new(rect.xMax - matrixWidth * 2f - 10f, yPos, matrixWidth, k_DefaultElementHeight);
		...
		private static void RuleMatrixOnGUI(Rect rect, RuleTile.TilingRule tilingRule)
		{
			...
			for (int y = 0; y <= 2; y++)
			{
				for (int x = 0; x <= 2; x++)
				{
					Rect r = new(rect.xMin + x * w, rect.yMin + y * h, w - 1, h - 1);
					if (x != 1 || y != 1)
					{
