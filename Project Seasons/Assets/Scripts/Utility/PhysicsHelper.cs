...
public class PhysicsHelper : MonoBehaviour
{
    ...
    static void Create()
    {
        GameObject physicsHelperGameObject = new("PhysicsHelper");
        s_Instance = physicsHelperGameObject.AddComponent<PhysicsHelper>();
    }

    Dictionary<Collider2D, MovingPlatform> m_MovingPlatformCache = new();
    Dictionary<Collider2D, PlatformEffector2D> m_PlatformEffectorCache = new();
    Dictionary<Collider2D, Tilemap> m_TilemapCache = new();
    Dictionary<Collider2D, AudioSurface> m_AudioSurfaceCache = new();
    ...
    {
        if (TryGetTilemap(collider, out Tilemap tilemap))
        {
            return tilemap.GetTile(tilemap.WorldToCell(position + direction * 0.4f));
        }

        if (TryGetAudioSurface(collider, out AudioSurface audioSurface))
        {
